...
int odometerRighDistance = odometerRight.getDistance();
int odometerLeftDistance = odometerLeft.getDistance();
...
void loop() {
  ...
  //----Prints what the sensors are reading, need to check it--//
...
void handleInput() {
  const int fSpeed = 62; //70% of the full speed forward
  const int bSpeed = -62; //70% of the full speed backward
  if (Serial.available()) {
    char input = Serial.read(); //read everything that has been received so far and log down the last entry
    switch (input) {

      case 'l':  //rotate counter-clockwise going forward, left
        car.setAngle(-75);
        car.setSpeed(fSpeed);
        delay(50);
        car.setSpeed(50);
      ...
      case 's': //for stopping the car in manual mode
        car.setSpeed(0);
        car.setAngle(0);
        break;
...
double detectSpotSize() {
  ...
  if  (rightSound.getDistance() <= 10 && odometerRighDistance > CAR_SIZE * 2 ||
       rightSound.getDistance() == 0 && odometerRighDistance > CAR_SIZE * 2) {
    park();
  }
}
